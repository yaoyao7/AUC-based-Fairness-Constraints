...
for k = 1:numiterSGD
    ...
    if mod(k,300)==1
        ...
        temp_constraintvalue = [a*(c^2+sum(Ax(sv==1).^2)/numfemale+sum(Ax(sv==0).^2)/nummale+2*c*sum(Ax(sv==1))/numfemale-2*c*sum(Ax(sv==0))/nummale-2*sum(Ax(sv==1))*sum(Ax(sv==0))/numfemale/nummale);
                            a*(c^2+sum(Ax(sv==0).^2)/nummale+sum(Ax(sv==1).^2)/numfemale-2*c*sum(Ax(sv==1))/numfemale+2*c*sum(Ax(sv==0))/nummale-2*sum(Ax(sv==0))*sum(Ax(sv==1))/nummale/numfemale)
                            ];
    ...
    gval = zeros(m,1);
    gval(1) = a*c^2+a*mymean((tempAx(tempsv==1)-aacst(1,1)).^2)+a*mymean((tempAx(tempsv==0)-bbcst(1,1)).^2);
    gval(1) = gval(1)-2*a*c*mymean(tempAx(tempsv==0))+2*a*c*mymean(tempAx(tempsv==1));
    gval(2) = a*c^2+a*mymean((tempAx(tempsv==0)-aacst(2,1)).^2)+a*mymean((tempAx(tempsv==1)-bbcst(2,1)).^2);
    gval(2) = gval(2)-2*a*c*mymean(tempAx(tempsv==1))+2*a*c*mymean(tempAx(tempsv==0));
    ...
    gvalalpha = zeros(m,1);
    gvalalpha(1,1) = 2*a*mymean(tempAx(tempsv==1)) - 2*a*mymean(tempAx(tempsv==0));
    gvalalpha(2,1) = 2*a*mymean(tempAx(tempsv==0)) - 2*a*mymean(tempAx(tempsv==1));
    ...
    gradientaacst=zeros(m,1);
    gradientbbcst=zeros(m,1);
    tempid_used=(tempsv==1);
    ...
    temp(2)=(balphacstsum(1,1)/etasum)^2/4/a+bysum(2)/etasum;
    temp(3)=(balphacstsum(2,1)/etasum)^2/4/a+bysum(3)/etasum;
    temp_U=max(temp);
    ...
    [~,~,~,AUC_g1] = perfcurve(sv,A*x,1);
    [~,~,~,AUC_valid] = perfcurve(b_valid,A_valid*x,1);
    [~,~,~,AUC_g1_valid] = perfcurve(sv_valid,A_valid*x,1);
    [~,~,~,AUC_test] = perfcurve(b_test,A_test*x,1);
    [~,~,~,AUC_g1_test] = perfcurve(sv_test,A_test*x,1);

    if abs(AUC_g1_valid-fair_con)<0.01
        train_auc_list=[train_auc_list;AUC];
        train_con1_list=[train_con1_list;AUC_g1];
        valid_auc_list=[valid_auc_list;AUC_valid];
        valid_con1_list=[valid_con1_list;AUC_g1_valid];
        test_auc_list=[test_auc_list;AUC_test];
        test_con1_list=[test_con1_list;AUC_g1_test];
    end
    ...
    tempy(2)=y(2)*exp((gval(1)-pcst(1,1))/(2*(1+B)^2/tau));
    tempy(3)=y(3)*exp((gval(2)-pcst(2,1))/(2*(1+B)^2/tau));
    y=tempy/sum(tempy);
