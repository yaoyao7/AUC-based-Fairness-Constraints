...
from loss_minmax import loss_con2
...
if True:
    ...
    for t in range(T):
        ...
        for j in range(outer_iter):
            ...
            for i in range(num_iter):
                ...
                z_lam_1 = single_cons(aa, cc, logits_y_pos_g_neg, logits_y_neg_g_pos, Loss.a.data[1], Loss.b.data[1], alpha[1]) + \
                          single_cons(aa, cc, logits_y_neg_g_neg, logits_y_pos_g_pos, Loss.a.data[2], Loss.b.data[2], alpha[2]) + \
                          mu*norm.data/2
                z_lam_1 -= c
                
                z_lam_2 = single_cons(aa, cc, logits_y_pos_g_pos, logits_y_neg_g_neg, Loss.a.data[3], Loss.b.data[3], alpha[3]) + \
                          single_cons(aa, cc, logits_y_neg_g_pos, logits_y_pos_g_neg, Loss.a.data[4], Loss.b.data[4], alpha[4]) + \
                          mu*norm.data/2
                z_lam_2 -= c
                
                z_alp_0 = 2*aa*(torch.mean(logits_y_neg)-torch.mean(logits_y_pos))
                z_alp_1 = 2*aa*(torch.mean(logits_y_pos_g_neg)-torch.mean(logits_y_neg_g_pos))
                z_alp_2 = 2*aa*(torch.mean(logits_y_neg_g_neg)-torch.mean(logits_y_pos_g_pos))
                z_alp_3 = 2*aa*(torch.mean(logits_y_pos_g_pos)-torch.mean(logits_y_neg_g_neg))
                z_alp_4 = 2*aa*(torch.mean(logits_y_neg_g_pos)-torch.mean(logits_y_pos_g_neg))
            ...
            # compute online upper bound
            u[0].data = ((sum_grad_alpha_bar[0]/sum_lr)**2)/4/aa + sum_grad_lamb[0]/sum_lr
            u[1].data = ((sum_grad_alpha_bar[1]/sum_lr)**2)/4/aa + ((sum_grad_alpha_bar[2]/sum_lr)**2)/4/aa + sum_grad_lamb[1]/sum_lr
            u[2].data = ((sum_grad_alpha_bar[3]/sum_lr)**2)/4/aa + ((sum_grad_alpha_bar[4]/sum_lr)**2)/4/aa + sum_grad_lamb[2]/sum_lr
            U = torch.max(u)
    ...
    s = "level_con2"+"_eps="+str(para.eps)+"_seed="+str(para.seed)+"_mu="+str(para.mu)+"_T="+str(para.T)+"_outer="+str(para.outer_iter)+"_inner="+str(para.num_iter)+".csv"
    d3.to_csv(s)   
