...
for k = 1:numiterSGD
    ...
    gval = zeros(m,1);
    gval(1) = a*c^2+a*mymean((tempAx(tempb==1 & tempsv==0)-aacst(1,1)).^2)+a*mymean((tempAx(tempb==-1 & tempsv==0)-bbcst(1,1)).^2);
    gval(1) = gval(1)-2*a*c*mymean(tempAx(tempb==-1 & tempsv==0))+2*a*c*mymean(tempAx(tempb==1 & tempsv==0));
    gval(1) = gval(1)+a*c^2+a*mymean((tempAx(tempb==1 & tempsv==1)-aacst(1,2)).^2)+a*mymean((tempAx(tempb==-1 & tempsv==1)-bbcst(1,2)).^2);
    gval(1) = gval(1)-2*a*c*mymean(tempAx(tempb==1 & tempsv==1))+2*a*c*mymean(tempAx(tempb==-1 & tempsv==1));
    gval(2) = a*c^2+a*mymean((tempAx(tempb==1 & tempsv==1)-aacst(2,1)).^2)+a*mymean((tempAx(tempb==-1 & tempsv==1)-bbcst(2,1)).^2);
    gval(2) = gval(2)-2*a*c*mymean(tempAx(tempb==-1 & tempsv==1))+2*a*c*mymean(tempAx(tempb==1 & tempsv==1));
    gval(2) = gval(2)+a*c^2+a*mymean((tempAx(tempb==1 & tempsv==0)-aacst(2,2)).^2)+a*mymean((tempAx(tempb==-1 & tempsv==0)-bbcst(2,2)).^2);
    gval(2) = gval(2)-2*a*c*mymean(tempAx(tempb==1 & tempsv==0))+2*a*c*mymean(tempAx(tempb==-1 & tempsv==0));
    ...
    gvalalpha = zeros(m,2);
    gvalalpha(1,1) = 2*a*mymean(tempAx(tempb==1 & tempsv==0)) - 2*a*mymean(tempAx(tempb==-1 & tempsv==0));
    gvalalpha(1,2) = 2*a*mymean(tempAx(tempb==-1 & tempsv==1)) - 2*a*mymean(tempAx(tempb==1 & tempsv==1));
    gvalalpha(2,1) = 2*a*mymean(tempAx(tempb==1 & tempsv==1)) - 2*a*mymean(tempAx(tempb==-1 & tempsv==1));
    gvalalpha(2,2) = 2*a*mymean(tempAx(tempb==-1 & tempsv==0)) - 2*a*mymean(tempAx(tempb==1 & tempsv==0));
    ...
    [~,~,~,AUCmale] = perfcurve(b(sv==0),A(sv==0,:)*x,1);
    [~,~,~,AUCfemale] = perfcurve(b(sv==1),A(sv==1,:)*x,1);
    
    [~,~,~,AUC_valid] = perfcurve(b_valid,A_valid*x,1);
    [~,~,~,AUCmale_valid] = perfcurve(b_valid(sv_valid==0),A_valid(sv_valid==0,:)*x,1);
    [~,~,~,AUCfemale_valid] = perfcurve(b_valid(sv_valid==1),A_valid(sv_valid==1,:)*x,1);
    [~,~,~,AUC_test] = perfcurve(b_test,A_test*x,1);
    [~,~,~,AUCmale_test] = perfcurve(b_test(sv_test==0),A_test(sv_test==0,:)*x,1);
    [~,~,~,AUCfemale_test] = perfcurve(b_test(sv_test==1),A_test(sv_test==1,:)*x,1);

    train_auc_list=[train_auc_list;AUC];
    train_con1_list=[train_con1_list;AUCmale];
    train_con2_list=[train_con2_list;AUCfemale];
    valid_auc_list=[valid_auc_list;AUC_valid];
    valid_con1_list=[valid_con1_list;AUCmale_valid];
    valid_con2_list=[train_con2_list;AUCfemale_valid];
    test_auc_list=[test_auc_list;AUC_test];
    test_con1_list=[test_con1_list;AUCmale_test];
    test_con2_list=[test_con2_list;AUCfemale_test];
    
    if abs(abs(AUCmale_valid-AUCfemale_valid)-fair_con)<0.01
        train_auc_list=[train_auc_list;AUC];
        train_con1_list=[train_con1_list;AUCmale];
        train_con2_list=[train_con2_list;AUCfemale];
        valid_auc_list=[valid_auc_list;AUC_valid];
