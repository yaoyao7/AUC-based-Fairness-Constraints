...
from loss_proxy import loss_con3
...
if True:
    ...
    for k in range(T):
        ...
        con1 = single_constraint_lamb(logits_y_pos_g_pos, logits_y_neg_g_pos) + \
               single_constraint_lamb(logits_y_neg_g_neg, logits_y_pos_g_neg)
        con2 = single_constraint_lamb(logits_y_pos_g_neg, logits_y_neg_g_neg) + \
               single_constraint_lamb(logits_y_neg_g_pos, logits_y_pos_g_pos)
        ...
        train_pred_group1 = list(torch.cat((train_pred_y_neg_g_pos,train_pred_y_pos_g_pos),0).cpu().numpy())
        train_pred_group2 = list(torch.cat((train_pred_y_pos_g_neg,train_pred_y_neg_g_neg),0).cpu().numpy())
        train_true_group1 = list(torch.cat((y_y_neg_g_pos,y_y_pos_g_pos),0).cpu().numpy())
        train_true_group2 = list(torch.cat((y_y_pos_g_neg,y_y_neg_g_neg),0).cpu().numpy())
        ...
        val_pred_group1 = list(torch.cat((val_pred_y_neg_g_pos,val_pred_y_pos_g_pos),0).cpu().numpy())
        val_pred_group2 = list(torch.cat((val_pred_y_pos_g_neg,val_pred_y_neg_g_neg),0).cpu().numpy())
        val_true_group1 = list(torch.cat((val_y_y_neg_g_pos, val_y_y_pos_g_pos),0).cpu().numpy())
        val_true_group2 = list(torch.cat((val_y_y_pos_g_neg, val_y_y_neg_g_neg),0).cpu().numpy())
    ...
    s = "proxy_con3_fair="+str(para.fair_con)+"_seed="+str(SEED)+"_lr_0="+str(para.lr_0)+"_T="+str(para.T)+"_c="+str(para.c)+".csv"
    d9.to_csv(s)